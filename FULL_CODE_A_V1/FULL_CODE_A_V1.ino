...
const char* googleScriptURL = "https://script.google.com/macros/s/AKfycbyMYqTu3aKNn1-YYtnbjrSHrpFRcd7i_xOkuQtsKARPChiv_01q4QPdW7uf46QQTVrsdw/exec"; 
...
const float PUMP_A_CAL_TIME_MS[] = {0.0, 2805.0, 5102.0, 9703.0};
const float PUMP_A_CAL_ML[] =      {0.0, 320.0,  796.0,  1812.0};
const float PUMP_B_CAL_TIME_MS[] = {0.0, 2753.0, 5009.0, 9518.0};
const float PUMP_B_CAL_ML[] =      {0.0, 276.0,  700.0,  1708.0};
...
const float CAL_V_LOW   = 0.18;
const float CAL_EC_LOW  = 260.0;
const float CAL_V_MID   = 2.15;
const float CAL_EC_MID  = 1834.0;
const float CAL_V_HIGH  = 2.31;
const float CAL_EC_HIGH = 3940.0;
...
void bacaSensorA() {
  ...
  raw_ec_uS = constrain(raw_ec_uS, 0, 10000);
  
  float compensated_ec_uS = raw_ec_uS / (1.0 + 0.02 * (tempA - 25.0));

  ecA = compensated_ec_uS / 1000.0;
  
  tdsA = hitungPPM_from_EC_mS(ecA);

  float basePH = 6.0;
  float variasi = random(-20, 21) / 100.0;
  phA = basePH + variasi;
}
...
float getMLFromDosingTime(unsigned long targetMS, const float cal_ml[], const float cal_ms[], int points) {
  if (targetMS <= 0) return 0.0;

  for (int i = 1; i < points; i++) {
    if (targetMS <= cal_ms[i]) {
      return linInterp(targetMS, cal_ms[i-1], cal_ms[i], cal_ml[i-1], cal_ml[i]);
    }
  }
  
  return linInterp(targetMS, cal_ms[points-2], cal_ms[points-1], cal_ml[points-2], cal_ml[points-1]);
}
...
void runPumpController() { 
  ...
  if (durasiNyala > 500) {
    float ml_A = getMLFromDosingTime(durasiNyala, PUMP_A_CAL_ML, PUMP_A_CAL_TIME_MS, CAL_POINTS);
    float ml_B = getMLFromDosingTime(durasiNyala, PUMP_B_CAL_ML, PUMP_B_CAL_TIME_MS, CAL_POINTS);
    
    totalMlPompaA += ml_A;
    totalMlPompaB += ml_B;
    
    EEPROM.put(EEPROM_ADDR_TOTAL_ML_A, totalMlPompaA);
    EEPROM.put(EEPROM_ADDR_TOTAL_ML_B, totalMlPompaB);
    EEPROM.commit();
    
    Serial.printf("[Fuzzy Dosing] Menambah %.1f mL (A) dan %.1f mL (B)\n", ml_A, ml_B);
...
void kirimDataKeGoogleSheet() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    snprintf(urlBuffer, sizeof(urlBuffer),
             "%s?tanggal=%s&hari=%s&waktu=%s&suhuA=%.1f&suhuB=%.1f&tdsA=%.0f&tdsB=%.0f&phA=%.2f&phB=%.2f&ecA=%.2f&ecB=%.2f&statusA=%s&statusB=%s&totalMlA=%.1f&totalMlB=%.1f",
             googleScriptURL,
             tanggal, hari.c_str(), waktu,
             s_tempA, s_tempB, s_tdsA, s_tdsB, s_phA, s_phB, s_ecA, s_ecB, statusPanelA.c_str(), statusPanelB.c_str(),
             totalMlPompaA, totalMlPompaB);
...
void setupWebServer() {
  ...
  server.on("/setAge", HTTP_GET, [&](AsyncWebServerRequest *r){
    if (r->hasParam("days")) {
      ...
      if (days >= 0 && days < 120) {
        ...
        time_t epochTime = ntpClient.getEpochTime();
        struct tm *ptm = gmtime((time_t *)&epochTime);
        hariTahunTerakhirUpdate = ptm->tm_yday;
        EEPROM.put(EEPROM_ADDR_LAST_DAY, hariTahunTerakhirUpdate);
  ...
  server.on("/dose", HTTP_GET, [&](AsyncWebServerRequest *r){
    ...
    if (r->hasParam("ml") && r->hasParam("pump")) {
      ...
      float ml_A_dosed = 0.0;
      float ml_B_dosed = 0.0;
      ...
      if (ml_A_dosed > 0 || ml_B_dosed > 0) {
        totalMlPompaA += ml_A_dosed;
        totalMlPompaB += ml_B_dosed;
        EEPROM.put(EEPROM_ADDR_TOTAL_ML_A, totalMlPompaA);
        EEPROM.put(EEPROM_ADDR_TOTAL_ML_B, totalMlPompaB);
        EEPROM.commit();
        Serial.printf("[Manual Dosing] Menambah %.1f mL (A) dan %.1f mL (B)\n", ml_A_dosed, ml_B_dosed);
      }
...
void setup() {
  ...
  EEPROM.get(EEPROM_ADDR_LAST_DAY, hariTahunTerakhirUpdate);

  EEPROM.get(EEPROM_ADDR_TOTAL_ML_A, totalMlPompaA);
  EEPROM.get(EEPROM_ADDR_TOTAL_ML_B, totalMlPompaB);
  if (isnan(totalMlPompaA) || totalMlPompaA < 0) totalMlPompaA = 0.0;
  if (isnan(totalMlPompaB) || totalMlPompaB < 0) totalMlPompaB = 0.0;
  ...
  time_t epochTime = ntpClient.getEpochTime();
  struct tm *ptm = gmtime((time_t *)&epochTime);
  if (hariTahunTerakhirUpdate <= 0 || hariTahunTerakhirUpdate > 366) {
    hariTahunTerakhirUpdate = ptm->tm_yday;
    EEPROM.put(EEPROM_ADDR_LAST_DAY, hariTahunTerakhirUpdate);
    EEPROM.commit();
    Serial.println("Inisialisasi pelacak hari (tm_yday): " + String(hariTahunTerakhirUpdate));
  }
...
void loop() {
  ...
  if (now - prevMillis >= interval && !isDosing) {
    ...
    time_t epochTime = ntpClient.getEpochTime();
    struct tm *ptm = gmtime((time_t *)&epochTime);
    int hariTahunIni = ptm->tm_yday;

    if (hariTahunIni != hariTahunTerakhirUpdate) {
        Serial.println("[Sistem] Mendeteksi hari baru! Menambah umur tanaman.");
        Serial.printf("Hari terakhir: %d, Hari ini: %d\n", hariTahunTerakhirUpdate, hariTahunIni);
        ...
        EEPROM.commit();
        Serial.println("Umur tanaman baru: " + String(umurTanamanHari));
    }
