 ...
 * Versi: FINAL - PENAMBAHAN POMPA AIR 2 & PERAPIAN KODE
 * Deskripsi: Implementasi sistem Fuzzy Logic Mamdani 3-input (Selisih PPM, Suhu, pH)
 * untuk mengontrol dosis nutrisi. Dashboard web untuk monitoring & kontrol.
 * PERUBAHAN:
 * - Menambahkan kontrol untuk Pompa Air 2 (Pin 26, SSR CH3).
 * - Merapikan struktur kode, penamaan variabel, dan menambah komentar.
 * - Memperbaiki sintaks pada JavaScript di dalam HTML (ternary operator).
 * - Metode pembuatan URL untuk Google Sheet diubah menjadi lebih robust (snprintf).
 * - Menambahkan sanitasi string pada JSON untuk web server.
 ******************************************************************************/
...
#define RELAY_PUMP_NUTRISI_PIN    25 // SSR CH4 (Pompa Nutrisi Utama)
#define RELAY_PUMP_AIR_2_PIN      26 // SSR CH3 (Pompa Air 2) <-- BARU

// =================================================================================
// --- PENGATURAN KALIBRASI SENSOR ---
// =================================================================================
// --- Kalibrasi pH ---
const bool FORCE_RESET_CAL_PH = false; // Set true untuk reset kalibrasi dari konstanta di bawah
...
// --- Variabel Sensor A (Lokal) ---
float phA = 0, tdsA = 0, tempA = 0, ecA = 0.0;

// --- Variabel Sensor B (Remote) ---
float phB = 0, tdsB = 0, tempB = 0, ecB = 0.0;

// --- Variabel Sistem & Fuzzy ---
...
const long interval = 15000; // Interval pembacaan sensor (15 detik)

// --- Variabel Kalibrasi pH dari EEPROM ---
float ph_slope = 0.0;
...
/**
 * @brief Interpolasi linear untuk menghitung nilai di antara dua titik.
 */
...
/**
 * @brief Membaca tegangan dari pin ADC dengan filter.
 * Mengambil banyak sampel, membuang nilai ekstrem, lalu merata-ratakan.
 */
float readVoltageADC(int pin) {
  ...
  // Sorting sederhana untuk menemukan nilai ekstrem
...
/**
 * @brief Membaca sensor suhu DS18B20.
 */
float readTemperatureSensor() {
  sensors.requestTemperatures();
  float t = sensors.getTempCByIndex(0);
  if (t == DEVICE_DISCONNECTED_C || t < -20 || t > 80) return 25.0; // Nilai default jika error
  return t;
}

/**
 * @brief Menghitung nilai EC (Electrical Conductivity) dari TDS.
 */
...
/**
 * @brief Menghitung nilai pH dari tegangan dengan kompensasi suhu.
 */
...
/**
 * @brief Membaca semua sensor yang terhubung ke ESP32 utama (Sensor A).
 */
void bacaSensorA() {
  ...
  if (measuredVoltage_TDS < VOLTAGE_THRESHOLD_TDS_DRY) {
    ...
    float tdsValue = 0.0;
    if (measuredVoltage_TDS <= VOLTAGE_MID_TDS) {
      tdsValue = linInterp(measuredVoltage_TDS, VOLTAGE_LOW_TDS, VOLTAGE_MID_TDS, PPM_LOW_TDS, PPM_MID_TDS);
    } else {
      tdsValue = linInterp(measuredVoltage_TDS, VOLTAGE_MID_TDS, VOLTAGE_HIGH_TDS, PPM_MID_TDS, PPM_HIGH_TDS);
    }
  ...
  if (v_ph >= VOLTAGE_THRESHOLD_PH_DRY) { 
    phA = 7.0; // Sensor kemungkinan tidak di dalam air
  } else if (ph_slope != 0.0) {
    phA = readpH(v_ph, tempA);
  } else {
    phA = 0.0; // Kalibrasi belum selesai
  }
}

/**
 * @brief Mengupdate data dari sensor B (remote) yang diterima via HTTP.
 */
...
/**
 * @brief Mengupdate tampilan pada LCD 16x2.
 */
void updateLCD16x2() {
  lcd.clear();
...
TargetNutrisi getTargetPPM(int hari) {
  ...
  else { target.ppm_bawah = 1500; target.ppm_atas = 1700; } // Default
  return target;
}
...
FuzzifiedValues fuzzifyAll(float ppm_terukur, float temp_celsius, float ph_level, TargetNutrisi target) {
  ...
  fv.deltaPPM[0] = trapMF(delta_ppm, -1000, -500, -500, -100); // Rendah
  fv.deltaPPM[1] = trapMF(delta_ppm, -250, -50, 50, 250);     // Ideal
  fv.deltaPPM[2] = trapMF(delta_ppm, 100, 500, 500, 1000);    // Tinggi

  fv.temperature[0] = trapMF(temp_celsius, 10.0, 15.0, 15.0, 24.0); // Dingin
  fv.temperature[1] = trapMF(temp_celsius, 22.0, 26.0, 30.0, 34.0); // Normal
  fv.temperature[2] = trapMF(temp_celsius, 32.0, 35.0, 35.0, 40.0); // Panas

  fv.pH[0] = trapMF(ph_level, 0.0, 4.5, 4.5, 5.5); // Asam
  fv.pH[1] = trapMF(ph_level, 5.2, 5.8, 6.5, 7.1); // Normal
  fv.pH[2] = trapMF(ph_level, 6.8, 7.5, 7.5, 14.0); // Basa
  return fv;
}

// Basis Aturan Fuzzy (Hanya 1 aturan aktif untuk contoh ini)
const float RULE_BASE[3][3][3] = {
  // [DeltaPPM][Suhu][pH] -> Dosis (0=Tidak, 0.5=Sedang, 1=Tinggi)
  { // DeltaPPM: RENDAH
    {0.5, 1.0, 0.5}, {0.5, 1.0, 0.5}, {0.0, 0.5, 0.0} // Suhu: Dingin, Normal, Panas
  },
  { // DeltaPPM: IDEAL
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      for (int k = 0; k < 3; k++) {
        ...
        denominator += alpha;
        if (alpha > 0.0) {
          max_dose = max(max_dose, z_value);
        }
      }
    }
  }

  // Tentukan pesan status berdasarkan aturan yang paling aktif
  if (max_dose == 1.0) {
    fuzzyStatusMessage = "Kebutuhan Nutrisi TINGGI. Kondisi Ideal.";
  } else if (max_dose == 0.5) {
    fuzzyStatusMessage = "Kebutuhan Nutrisi SEDANG. Kondisi Kurang Optimal.";
  } else {
    fuzzyStatusMessage = "Tidak Perlu Dosis. TDS Cukup/Tinggi.";
  }

  if (denominator == 0) return 0.0;
  float crisp_output = numerator / denominator; // Center of Gravity (COG)
  Serial.printf(" -> COG Crisp Output: %.3f\n", crisp_output);
  return crisp_output;
}

/**
 * @brief Kontroler utama yang menjalankan logika fuzzy dan mengendalikan pompa.
 */
void logicController() {
  ...
  // Relay aktif saat LOW
  uint8_t relayState = nyalakanPompa ? LOW : HIGH; 
  digitalWrite(RELAY_PUMP_NUTRISI_PIN, relayState);
}
...
/**
 * @brief Mengirim data sensor ke Google Sheet melalui Apps Script.
 */
void kirimDataKeGoogleSheet() {
  if (WiFi.status() == WL_CONNECTED) {
    // Sanitasi data untuk mencegah nilai NaN atau Inf
    ...
    // Gunakan snprintf untuk membuat URL dengan aman
    char urlBuffer[512];
    ...
    if (httpCode > 0) {
      ...
      if (httpCode != HTTP_CODE_OK && httpCode != 302) { 
         ...
         Serial.println("Respons Server:"); Serial.println(payload);
      }
    } else {
...
/**
 * @brief Menangani perintah dari Serial Monitor (misal: 'v' untuk cek tegangan).
 */
...
/**
 * @brief Mengatur semua endpoint untuk web server.
 */
void setupWebServer() {
  // Endpoint utama untuk menampilkan dashboard
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *r){
    r->send_P(200, "text/html", index_html);
  });
  
  // Endpoint untuk menyediakan data JSON ke dashboard
  server.on("/data", HTTP_GET, [](AsyncWebServerRequest *r){
    ...
    // Sanitasi string untuk mencegah JSON rusak
    ...
    json += "\"pump1State\":" + String(digitalRead(RELAY_PUMP_NUTRISI_PIN) == LOW ? "true" : "false") + ",";
    json += "\"pump2State\":" + String(digitalRead(RELAY_PUMP_AIR_2_PIN) == LOW ? "true" : "false") + ","; // <-- BARU
  ...
  // Endpoint untuk menerima data dari Sensor B
  ...
  // Endpoint untuk mengontrol relay (pompa)
  server.on("/relay", HTTP_GET, [](AsyncWebServerRequest *r) {
    if (r->hasParam("id")) {
      ...
      if (pumpId == 1) {
        pinToToggle = RELAY_PUMP_NUTRISI_PIN;
      } else if (pumpId == 2) {
        pinToToggle = RELAY_PUMP_AIR_2_PIN; // <-- BARU
      }

      if (pinToToggle != 0) {
        uint8_t currentState = digitalRead(pinToToggle);
        digitalWrite(pinToToggle, !currentState);
        modeOtomatis = false; // Setiap kontrol manual akan menonaktifkan mode otomatis
        r->send(200, "text/plain", "OK");
        return;
      }
    }
  ...
  // Endpoint untuk mengubah mode (Otomatis/Manual)
...
void setup() {
  ...
  if (FORCE_RESET_CAL_PH) {
    ...
    // Jika data EEPROM tidak valid, gunakan nilai default dari konstanta
  ...
  pinMode(RELAY_PUMP_NUTRISI_PIN, OUTPUT);
  pinMode(RELAY_PUMP_AIR_2_PIN, OUTPUT); // <-- BARU
  digitalWrite(RELAY_PUMP_NUTRISI_PIN, HIGH); // HIGH = Mati
  digitalWrite(RELAY_PUMP_AIR_2_PIN, HIGH);  // HIGH = Mati <-- BARU
...
void loop() {
  ...
  // Blok ini hanya berjalan setiap 'interval' (15 detik)
  if (now - prevMillis >= interval) {
    ...
    // Jalankan logika fuzzy hanya jika mode otomatis aktif
    if (modeOtomatis) {
      logicController();
    } else {
      // Jika mode manual, matikan pompa nutrisi yang dikontrol Fuzzy
      digitalWrite(RELAY_PUMP_NUTRISI_PIN, HIGH); 
  ...
  // Cek perintah dari serial setiap saat
