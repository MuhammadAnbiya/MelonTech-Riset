...
const long interval = 15000; // Interval pembacaan dan pengiriman data (15 detik)
...
void kirimDataKeGoogleSheet() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode > 0) {
      Serial.println("Kode Balasan: " + String(httpCode));
    } else {
      Serial.println("Gagal mengirim request ke Google Sheet.");
    }
    http.end();
  }
}
