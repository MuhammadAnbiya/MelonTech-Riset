 ...
 * Versi: FINAL + EC ONLY FUZZY CONTROL
 * Deskripsi: Implementasi sistem Fuzzy Logic yang disederhanakan, di mana
 * HANYA nilai EC yang menjadi pemicu untuk penambahan nutrisi.
 * PERUBAHAN UTAMA:
 * - Menggunakan library "Fuzzy.h" untuk implementasi yang lebih standar.
 * - Logika fuzzy kini hanya memiliki 1 input: selisih EC (Error_EC).
 * - Sensor Suhu dan pH tidak lagi mempengaruhi keputusan dosis.
 * - Output fuzzy kini berupa durasi pompa, bukan sekadar ON/OFF.
 ******************************************************************************/
...
#include <EEPROM.h>
#include <Fuzzy.h> // <-- PERUBAHAN: Library baru untuk Fuzzy Logic

// =================================================================================
// --- OBJEK & VARIABEL GLOBAL ---
// =================================================================================
// ... (Bagian Konfigurasi, Pinout, Kalibrasi tetap sama) ...
...
const float PUMP_A_FLOW_RATE_ML_S = 214.28;
const float PUMP_B_FLOW_RATE_ML_S = 125.0;
const float PUMP_ACTIVATION_DELAY_S = 0.5;
const float PPM_TO_EC_CONVERSION_FACTOR = 700.0;

Fuzzy *fuzzy = new Fuzzy(); // <-- PERUBAHAN: Objek Fuzzy Logic
...
void updateSensorB(float ph, float tds, float temp) { phB = ph; tdsB = tds; tempB = temp; ecB = hitungEC_from_TDS(tdsB); }
...
// =================================================================================
// --- BAGIAN LOGIKA FUZZY (HANYA BERBASIS EC) ---
// =================================================================================
struct TargetNutrisi { 
  float ec_bawah; 
  float ec_atas; 
};

TargetNutrisi getTargetNutrisi(int hari) { 
...
// <-- PERUBAHAN: Fungsi setup untuk logika fuzzy yang baru
void setupFuzzy() {
  // 1. INPUT: Error_EC (Target - Aktual)
  FuzzyInput *errorEC = new FuzzyInput(1);

  // Fungsi Keanggotaan untuk Error_EC
  FuzzySet *tinggi = new FuzzySet(-2.0, -2.0, -1.0, -0.3); // EC berlebih
  errorEC->addFuzzySet(tinggi);
  FuzzySet *ideal = new FuzzySet(-0.3, -0.1, 0.1, 0.3); // EC sesuai
  errorEC->addFuzzySet(ideal);
  FuzzySet *rendah = new FuzzySet(0.1, 0.3, 0.7, 0.9); // EC kurang
  errorEC->addFuzzySet(rendah);
  FuzzySet *sangatRendah = new FuzzySet(0.7, 1.0, 2.0, 2.0); // EC sangat kurang
  errorEC->addFuzzySet(sangatRendah);

  fuzzy->addFuzzyInput(errorEC);

  // 2. OUTPUT: Durasi Dosis Pompa (dalam ms)
  ...
  fuzzy->addFuzzyOutput(durasiDosis);

  // 3. ATURAN (RULE BASE)
  FuzzyRule *rule1 = new FuzzyRule(1, IF(tinggi, mati), mati);
  fuzzy->addFuzzyRule(rule1);
  FuzzyRule *rule2 = new FuzzyRule(2, IF(ideal, mati), mati);
  fuzzy->addFuzzyRule(rule2);
  FuzzyRule *rule3 = new FuzzyRule(3, IF(rendah, sedang), sedang);
  fuzzy->addFuzzyRule(rule3);
  FuzzyRule *rule4 = new FuzzyRule(4, IF(sangatRendah, lama), lama);
  fuzzy->addFuzzyRule(rule4);
}

// <-- PERUBAHAN: logicController sepenuhnya ditulis ulang
void logicController() { 
  if (isDosing) { // Jangan jalankan logika baru jika sedang melakukan dosis
    fuzzyStatusMessage = "Sedang Dosis Otomatis...";
    return;
  }

  TargetNutrisi target = getTargetNutrisi(umurTanamanHari); 
  fuzzyTargetMessage = "EC " + String(target.ec_bawah, 1) + "-" + String(target.ec_atas, 1);
  
  float target_mid_ec = (target.ec_bawah + target.ec_atas) / 2.0;
  float error_ec = target_mid_ec - ecA; // Error positif = EC kurang

  // Jalankan sistem Fuzzy
  fuzzy->setInput(1, error_ec);
  fuzzy->fuzzify();
  long durasiNyala = fuzzy->defuzzify(1);

  Serial.printf("Target EC: %.2f, Aktual: %.2f, Error: %.2f -> Durasi Dosis: %d ms\n", 
                target_mid_ec, ecA, error_ec, durasiNyala);

  // Tentukan pesan status berdasarkan durasi
  if (durasiNyala > 7500) { // Cenderung ke 'Lama'
    fuzzyStatusMessage = "EC Sangat Rendah. Dosis LAMA.";
    fuzzyDecisionMessage = "Memicu Dosis " + String(durasiNyala) + " ms";
  } else if (durasiNyala > 1000) { // Cenderung ke 'Sedang'
    fuzzyStatusMessage = "EC Rendah. Dosis SEDANG.";
    fuzzyDecisionMessage = "Memicu Dosis " + String(durasiNyala) + " ms";
  } else { // Cenderung ke 'Mati'
    fuzzyStatusMessage = "EC Ideal atau Tinggi.";
    fuzzyDecisionMessage = "Tidak Perlu Dosis";
  }
  
  // Aksi berdasarkan hasil defuzzifikasi
  if (durasiNyala > 500) { // Beri sedikit ambang batas untuk menghindari noise
    isDosing = true;
    dosingStartTime = millis();
    dosingDuration = durasiNyala;
    dosingPumpA = true;
    dosingPumpB = true;
...
// ... (Sisa kode Web Server, Setup, Loop tetap sama strukturnya) ...
// =================================================================================
// --- KOMUNIKASI & WEB SERVER ---
// =================================================================================
void kirimDataKeGoogleSheet() { /* ... (Tidak ada perubahan) ... */ }
void handleSerialCommands() { /* ... (Tidak ada perubahan) ... */ }
void setupWebServer() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *r){ r->send_P(200, "text/html", index_html); });
  
  server.on("/data", HTTP_GET, [](AsyncWebServerRequest *r){
    // ...
    String safeTargetNutrisi = fuzzyTargetMessage; safeTargetNutrisi.replace("\"", "\\\"");
    String json = "{";
    // ...
    json += "\"targetNutrisi\":\"" + safeTargetNutrisi + "\",";
    // ...
...
void setup() {
  ...
  setupFuzzy(); // <-- PERUBAHAN: Inisialisasi sistem fuzzy saat startup

  EEPROM.get(EEPROM_ADDR_PLANT_AGE, umurTanamanHari);
  Serial.printf("Membaca umur tanaman dari EEPROM: %d hari\n", umurTanamanHari);
  if (umurTanamanHari <= 0 || umurTanamanHari > 120) {
    umurTanamanHari = 1;
    EEPROM.put(EEPROM_ADDR_PLANT_AGE, umurTanamanHari);
    EEPROM.commit();
  }
  // ... (Sisa fungsi setup tidak ada perubahan)
}
...
void loop() {
  ...
  if (now - prevMillis >= interval && !isDosing) { // <-- PERUBAHAN: Logika utama hanya berjalan jika tidak sedang dosis
    prevMillis = now;
    Serial.println("\n--- Membaca Sensor ---");
    bacaSensorA();
    ...
    if (modeOtomatis) {
      ...
      fuzzyTargetMessage = "N/A";
      fuzzyDecisionMessage = "Mode Manual";
    }
  }

  // Logika untuk mematikan pompa setelah durasi dosis selesai
  if (isDosing) {
