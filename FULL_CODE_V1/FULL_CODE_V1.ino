 ...
 * Versi: FINAL + EC FUZZY + DOSING AKURASI TINGGI
 * Deskripsi: Implementasi sistem Fuzzy Logic dengan kalibrasi pompa presisi
 * yang memperhitungkan waktu tunda (startup delay).
 * PERUBAHAN UTAMA:
 * - Kalibrasi flow rate pompa A & B diperbarui dengan 10 data eksperimen baru.
 * - Menambahkan konstanta PUMP_STARTUP_DELAY_S untuk kompensasi waktu tunda,
 * meningkatkan akurasi dosis untuk volume kecil (mL).
 * - Logika dosis manual (/dose) telah diperbarui untuk menggunakan kompensasi ini.
 ******************************************************************************/
...
// --- PERUBAHAN BARU: Kalibrasi Debit dan Waktu Tunda ---
// Berdasarkan data eksperimen terbaru (2000 mL)
const float PUMP_A_FLOW_RATE_ML_S = 217.25; // Hasil: 2000ml / 9.206s
const float PUMP_B_FLOW_RATE_ML_S = 221.89; // Hasil: 2000ml / 9.013s

// Waktu tunda (detik) dari pompa ON hingga air keluar dari selang 2.5m.
// SESUAIKAN NILAI INI dengan menguji dosis kecil (misal 50mL) untuk akurasi maksimal.
...
bool isDosing = false;
unsigned long dosingStopA = 0; // Waktu berhenti individu untuk Pompa A
unsigned long dosingStopB = 0; // Waktu berhenti individu untuk Pompa B
...
void setupFuzzy() {
  ...
  // --- PERBAIKAN: Menggunakan sintaks FuzzyRule yang benar ---
  ...
  fuzzy->addFuzzyRule(new FuzzyRule(1, if_ec_tinggi, then_mati));

  FuzzyRuleAntecedent *if_ec_ideal = new FuzzyRuleAntecedent();
  if_ec_ideal->joinSingle(ideal);
  fuzzy->addFuzzyRule(new FuzzyRule(2, if_ec_ideal, then_mati));
  ...
  fuzzy->addFuzzyRule(new FuzzyRule(3, if_ec_rendah, then_sedang));
  ...
  fuzzy->addFuzzyRule(new FuzzyRule(4, if_ec_sangat_rendah, then_lama));
}
...
void setupWebServer() {
  ...
  server.on("/dose", HTTP_GET, [](AsyncWebServerRequest *r){
    ...
    if (r->hasParam("ml") && r->hasParam("pump")) {
      ...
      // --- PERUBAHAN BARU: Menambahkan PUMP_STARTUP_DELAY_S ke dalam kalkulasi ---
